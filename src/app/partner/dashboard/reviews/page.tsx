...
export default function PartnerReviewsPage() {
  ...
  const fetchReviews = useCallback(async () => {
    setIsLoading(true)
    try {
      const { data } = await axios.get('/api/partner/reviews', {
        params: {
          page,
          limit,
        },
  ...
  const handleDelete = async (id: number) => {
    ...
    setIsDeleting(id)
    try {
      await axios.delete('/api/partner/reviews', { params: { id } })
  ...
  const handleToggleApproval = async (review: ProductReview) => {
    ...
    try {
      await axios.put('/api/partner/reviews', {
        id: review.id,
  ...
  if (isLoading) {
    return (
      <div className="p-6 space-y-6 w-full max-w-7xl mx-auto">
        <h1 className="text-2xl font-bold text-green-500">Product Reviews</h1>
