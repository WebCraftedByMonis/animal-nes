...
interface Product {
  ...
  quantity: number;
  // add any additional fields if exist
}
...
export default function AdminOrdersPage() {
  ...
  const handleStatusToggle = async (orderId: string) => {
    try {
      ...
      setOrders((prev) =>
        prev.map((order) =>
          order.id ? { ...order, status: 'delivered' } : order
        )
      )
    } catch (err) {
      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            <TableHead>Animal quantity</TableHead>
              ...
              <TableRow key={`${order.id}-${idx}`}>
                ...
                {idx === 0 && (
                  <>
                    ...
                    <TableCell rowSpan={order.items.length}>{order.paymentMethod}</TableCell>
                ...
                <TableCell>{item.animal?.quantity ?? '-'}</TableCell>
                <TableCell>
                  {item.product ? item.quantity - (item.animal?.quantity ?? 0) : '-'}
                </TableCell>
                ...
                {idx === 0 && (
                  <TableCell rowSpan={order.items.length}>PKR {order.total.toFixed(2)}</TableCell>
                )}
              </TableRow>
            ))
          ))}
