...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const MASTER_TRAINER_FEE = 5000; // PKR
    try {
      await prisma.transaction.create({
        data: {
          type: 'OTHER_INCOME',
          amount: MASTER_TRAINER_FEE,
          description: `Master Trainer Program Registration Payment from ${registration.name}`,
          transactionDate: new Date(),
          paymentMethod: 'online_transfer', // Based on payment screenshot
          status: 'COMPLETED',
          isAutoGenerated: true,
          notes: `Auto-generated from master trainer registration #${registrationId}. Email: ${registration.email}, WhatsApp: ${registration.whatsappNumber}`,
        },
      });
