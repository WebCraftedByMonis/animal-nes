...
export async function POST(request: NextRequest) {
  try {
    ...
    // ✅ Distribute consultation fee payment (70% to doctor, 30% to revenue)
    if (history.appointment?.paymentInfo && history.appointment.paymentInfo.consultationFee > 0) {
      const totalAmount = history.appointment.paymentInfo.consultationFee;
      const doctorShare = totalAmount * 0.7; // 70% to doctor
      const revenueShare = totalAmount * 0.3; // 30% to revenue

      console.log(`[Payment Distribution] Consultation Fee for Appointment #${history.appointmentId}:`);
      console.log(`  Total Amount: Rs. ${totalAmount.toFixed(2)}`);
      console.log(`  Doctor Share (70%): Rs. ${doctorShare.toFixed(2)}`);
      ...
      if (doctor) {
        ...
        console.log(`  ✅ Added Rs. ${doctorShare.toFixed(2)} to Dr. ${doctor.partnerName}'s wallet`);
        ...
        await prisma.transaction.updateMany({
          ...
          data: {
            amount: revenueShare,
            notes: `Consultation Fee - Total: Rs. ${totalAmount.toFixed(2)} | Doctor Share (70%): Rs. ${doctorShare.toFixed(2)} | Revenue Share (30%): Rs. ${revenueShare.toFixed(2)}`,
            updatedAt: new Date()
          }
        });

        console.log(`  ✅ Updated transaction to show Rs. ${revenueShare.toFixed(2)} revenue (30%)`);
      } else {
