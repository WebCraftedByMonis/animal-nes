...
// ── Parse next page URL from Shopify Link header ───────────────────────────────
...
async function fetchShopifyProducts(origin: string, collectionHandle: string): Promise<ExtractedProduct[]> {
  ...
  while (url) {
    ...
    for (const item of data.products) {
      ...
      // Full description from body_html (Shopify gives us the complete HTML)
      const description = htmlToText(item.body_html || "").substring(0, 3000);

      // Variants: each has a title (size) and price
      ...
      // If only "Default Title" variant, treat as single product with just price
...
// Shopify body_html from the API is usually complete, but the detail page
// may have extra sections (tabs, accordions) added by the theme.
function parseDetailPage(html: string, apiDescription: string): { description: string; dosage: string } {
  ...
  // Try common Shopify theme description containers
  const selectors = [
  ...
  // Use whichever is longer: API body_html or page description
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Support both /collections/HANDLE and /collections/HANDLE/products
    ...
    console.log(`[PETSGROCERY] origin=${origin}, collection=${collectionHandle}`);
    ...
    // Enrich with detail pages in batches of 3 (for fuller description)
    ...
    for (let i = 0; i < apiProducts.length; i += 3) {
      ...
      const results = await Promise.all(batch.map(async (p) => {
        ...
        return {
          ...
          dosage:      dosage      || p.dosage,
        } as ExtractedProduct;
      }));
      results.forEach(r => enriched.push(r));
      console.log(`[PETSGROCERY] batch ${Math.floor(i / 3) + 1}, total: ${enriched.length}`);
    }
