    ...
    try {
      ...
      const { data } = await axios.get('/api/partner', {
        params: { 
          ...
          page, 
          limit,
          partnerType: 'sales' // Filter for sales partners
        },
      })
      
      // Filter for sales & marketing partners
      const salesPartners = data.data.filter((partner: Partner) => 
        partner.partnerType?.toLowerCase() === 'sales' || 
        partner.partnerType?.toLowerCase() === 'marketing' ||
        partner.partnerType?.toLowerCase() === 'sales and marketing' ||
        partner.partnerType?.toLowerCase() === 'sales & marketing' ||
        (partner.shopName && !partner.specialization && !partner.qualificationDegree) // Has shop but no medical qualifications
      )
      
      setPartners(salesPartners)
      setTotal(salesPartners.length)
    } catch (error) {
      console.log(error)
