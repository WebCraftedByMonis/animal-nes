...
export default function FarmersClient() {
  ...
  const updateURL = useCallback((newPage: number, newSearch: string, newSpec: string) => {
    ...
    router.push(queryString ? `/Farmers?${queryString}` : '/Farmers', { scroll: false })
  }, [router])
    ...
    <div className="max-w-7xl mx-auto p-6 space-y-6">
      ...
      {partners.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-xl text-gray-600 dark:text-gray-400 mb-2">No farmers found</p>
          <p className="text-gray-500 dark:text-gray-500">Try adjusting your search criteria</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {partners.map((partner) => (
            <Link key={partner.id} href={`/Farmers/${partner.id}`}>
