...
export default function MuslimherbalExtractor() {
  ...
  const handleExtract = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/scrape-products-muslimherbal", {
        method: "POST",
  ...
  return (
    <div className="max-w-full">
      <p className="text-gray-500 text-sm mb-1">
        Extract products from <span className="font-semibold text-gray-700">muslimherbalandnutraceutical.com</span> and add them directly to your store.
      </p>
      <p className="text-xs text-gray-400 mb-6">
        This extractor is configured for <span className="font-medium">Muslim Herbal &amp; Nutraceutical</span> (WooCommerce) — paste the shop or any category URL to extract products with variants and descriptions.
      </p>
      ...
      <form onSubmit={handleExtract} className="bg-gray-50 border border-gray-200 rounded-xl p-4 mb-6 space-y-4">
        ...
        <div className="flex flex-wrap gap-3 items-end">
          <div className="flex-1 min-w-[260px]">
            <label className="block text-xs font-medium text-gray-600 mb-1">Shop / Category URL *</label>
            <div className="relative">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
              <input
                type="url" value={url} onChange={e => setUrl(e.target.value)}
                placeholder="https://muslimherbalandnutraceutical.com/shop/"
      ...
      {loading && (
        <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg text-blue-700 text-sm flex items-center gap-2">
          ...
          Fetching products via WooCommerce API → enriching variants and descriptions from product pages…
        </div>
      )}
      ...
      {!loading && !rows.length && !fetchError && (
        <div className="text-center py-16 text-gray-400">
          ...
          <p className="text-sm">Enter the shop or a category URL above to get started.</p>
          <p className="text-xs mt-1 opacity-70">e.g. https://muslimherbalandnutraceutical.com/shop/</p>
        </div>
      )}
    </div>
  );
