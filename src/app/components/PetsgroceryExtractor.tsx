...
export default function PetsgroceryExtractor() {
  ...
  const handleExtract = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/scrape-products-petsgrocery", {
        method: "POST",
  ...
  return (
    <div className="max-w-full">
      <p className="text-gray-500 text-sm mb-1">
        Extract products from <span className="font-semibold text-gray-700">petsgrocery.pk</span> and add them directly to your store.
      </p>
      <p className="text-xs text-gray-400 mb-6">
        This extractor is configured for <span className="font-medium">PetsGrocery PK</span> (Shopify) — paste any collection URL to extract products with all variants, prices and full descriptions.
      </p>
      ...
      <form onSubmit={handleExtract} className="bg-gray-50 border border-gray-200 rounded-xl p-4 mb-6 space-y-4">
        ...
        <div className="flex flex-wrap gap-3 items-end">
          <div className="flex-1 min-w-[260px]">
            <label className="block text-xs font-medium text-gray-600 mb-1">Collection URL *</label>
            <div className="relative">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
              <input
                type="url" value={url} onChange={e => setUrl(e.target.value)}
                placeholder="https://petsgrocery.pk/collections/cat-products"
      ...
      {loading && (
        <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg text-blue-700 text-sm flex items-center gap-2">
          ...
          Fetching products via Shopify API → enriching descriptions from product pages…
        </div>
      )}
      ...
      {!loading && !rows.length && !fetchError && (
        <div className="text-center py-16 text-gray-400">
          ...
          <p className="text-sm">Enter a PetsGrocery collection URL above to get started.</p>
          <p className="text-xs mt-1 opacity-70">e.g. https://petsgrocery.pk/collections/cat-products</p>
        </div>
      )}
    </div>
  );
