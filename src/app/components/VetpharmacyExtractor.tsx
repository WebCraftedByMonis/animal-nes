...
export default function VetpharmacyExtractor() {
  ...
  const handleExtract = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/scrape-products-vetpharmacy", {
        method: "POST",
  ...
  return (
    <div className="max-w-full">
      <p className="text-gray-500 text-sm mb-1">
        Extract products from <span className="font-semibold text-gray-700">vetpharmacy.ae</span> and add them directly to your store.
      </p>
      <p className="text-xs text-gray-400 mb-6">
        This extractor is configured for <span className="font-medium">VetPharmacy</span> (Odoo) — paste the shop or category URL to extract all products.
      </p>
      ...
      <form onSubmit={handleExtract} className="bg-gray-50 border border-gray-200 rounded-xl p-4 mb-6 space-y-4">
        ...
        <div className="flex flex-wrap gap-3 items-end">
          <div className="flex-1 min-w-[260px]">
            <label className="block text-xs font-medium text-gray-600 mb-1">Shop URL *</label>
            <div className="relative">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
              <input
                type="url" value={url} onChange={e => setUrl(e.target.value)}
                placeholder="https://vetpharmacy.ae/shop"
      ...
      {loading && (
        <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg text-blue-700 text-sm flex items-center gap-2">
          ...
          Fetching products from VetPharmacy (Odoo) — this may take a while for large catalogs…
        </div>
      )}
      ...
      {!loading && !rows.length && !fetchError && (
        <div className="text-center py-16 text-gray-400">
          ...
          <p className="text-sm">Enter a VetPharmacy shop URL above to get started.</p>
          <p className="text-xs mt-1 opacity-70">e.g. https://vetpharmacy.ae/shop</p>
        </div>
      )}
    </div>
  );
